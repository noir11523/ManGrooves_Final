            ...
            <div class="auth-modal-main">
                ...
                <div class="tab-content pt-4">
                    ...
                    <div class="tab-pane fade" id="modal-register" role="tabpanel" aria-labelledby="modal-register-tab" tabindex="0">
                        ...
                        <?php if (($barangays ?? []) === []): ?>
                            <div class="alert alert-danger" role="alert">Registration is temporarily unavailable because the barangay list could not be loaded. Please try again shortly.</div>
                        <?php endif; ?>
                        <form method="post" action="<?= e(url('register.php')) ?>" class="needs-validation" novalidate>
                            <?= Csrf::field() ?>
                            <div class="row g-3">
                                <div class="col-sm-6"><label class="form-label" for="modal-first-name">First name</label><input class="form-control" id="modal-first-name" name="first_name" type="text" autocomplete="given-name" maxlength="60" required><div class="invalid-feedback">Enter your first name.</div></div>
                                <div class="col-sm-6"><label class="form-label" for="modal-last-name">Last name</label><input class="form-control" id="modal-last-name" name="last_name" type="text" autocomplete="family-name" maxlength="59" required><div class="invalid-feedback">Enter your last name.</div></div>
                                ...
                                <div class="col-sm-6"><label class="form-label" for="modal-barangay">Barangay</label><select class="form-select" id="modal-barangay" name="barangay_id" aria-describedby="modal-barangay-help" required <?= ($barangays ?? []) === [] ? 'disabled' : '' ?>><option value="">Choose…</option><?php foreach (($barangays ?? []) as $barangay): ?><option value="<?= e((string) $barangay['id']) ?>" <?= count($barangays) === 1 ? 'selected' : '' ?>><?= e((string) $barangay['name']) ?>, <?= e((string) $barangay['city_municipality']) ?></option><?php endforeach; ?></select><div class="form-text" id="modal-barangay-help">The current pilot site is Barangay Inayawan, Cebu City.</div></div>
                                <div class="col-sm-6"><label class="form-label" for="modal-new-password">Password</label><div class="input-group password-group"><input class="form-control" id="modal-new-password" name="password" type="password" autocomplete="new-password" minlength="8" maxlength="72" required data-password-strength><button class="btn password-toggle" type="button" data-password-toggle="#modal-new-password" aria-label="Show password"><i class="bi bi-eye" aria-hidden="true"></i></button></div><div class="form-text">Use 8 to 72 characters.</div></div>
                                <div class="col-sm-6"><label class="form-label" for="modal-confirm-password">Confirm password</label><div class="input-group password-group"><input class="form-control" id="modal-confirm-password" name="password_confirmation" type="password" autocomplete="new-password" minlength="8" maxlength="72" required data-password-confirm="#modal-new-password"><button class="btn password-toggle" type="button" data-password-toggle="#modal-confirm-password" aria-label="Show confirm password"><i class="bi bi-eye" aria-hidden="true"></i></button></div></div>
                                <div class="col-12"><div class="form-check"><input class="form-check-input" id="modal-privacy" name="privacy_consent" type="checkbox" value="1" required><label class="form-check-label" for="modal-privacy">I have read the <a href="<?= e(url('privacy.php')) ?>" data-privacy-notice>privacy notice</a> and consent to the described use of my account and field-report data.</label></div></div>
